...
/**Extern variable defined in counter.h by usman
*/
int machinecycles=0;
...
{ 	
	/**Added by usman (To activate include device.h and kernel.h)
	const struct counter_stm32_config *config = dev->config;
	printk("Machine cycles = %d\n", machinecycles);
	printk("irqn = %d ", config->irqn);
	printk(irq_is_enabled(config->irqn)?"Enabled\n":"NotEnabled\n");
	printk("Machine cycles = %d\n", machinecycles);
	End by usman
	*/
...
static int counter_stm32_init_timer(const struct device *dev)
{
	...
	/*Below 2 lines added by usman */
	LL_TIM_EnableIT_UPDATE(cfg->timer); //Enable update interrupt UIE set 	
    LL_TIM_GenerateEvent_UPDATE(cfg->timer); // Force update generation (UG = 1)
...
/** Commented by usman: No need for such complexity
...
void counter_stm32_irq_handler(const struct device *dev)
{
	...
	if (LL_TIM_IsActiveFlag_UPDATE(timer) && LL_TIM_IsEnabledIT_UPDATE(timer)) {
		LL_TIM_ClearFlag_UPDATE(timer);
	//Below variable added by usman
	machinecycles++;
	//counter_stm32_top_irq_handle(dev); commended by usman
	}
}
